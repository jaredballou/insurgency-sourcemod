...
static DataPackPos:PluginPack_BaseURL;
...
PluginToIndex(Handle:plugin) {
	...
	for (new i = 0; i < maxPlugins; i++) {
		new Handle:hPluginPack = GetArrayCell(g_hPluginPacks, i);
...
Updater_AddPlugin(Handle:plugin, const String:url[]) {	
	...
	if (index != -1) {
		...
		new DataPack:hPluginPack = CreateDataPack();
		...
		Format(sBaseURL, sizeof(sBaseURL), url);
		StripPathFilename(sBaseURL);
		PluginPack_BaseURL = GetPackPosition(hPluginPack);
		WritePackString(hPluginPack, sBaseURL);
...
Updater_GetBaseURL(index, String:buffer[], size) {
	new Handle:hPluginPack = GetArrayCell(g_hPluginPacks, index);
	SetPackPosition(hPluginPack, PluginPack_BaseURL);
	ReadPackString(hPluginPack, buffer, size);
}

Updater_SetBaseURL(index, const String:sBaseURL[]) {
	PrintToServer("[UPDATER] Setting BaseURL for %d to %s", index, sBaseURL);
	new Handle:hPluginPack = GetArrayCell(g_hPluginPacks, index);
	SetPackPosition(hPluginPack, PluginPack_BaseURL);
	WritePackString(hPluginPack, sBaseURL);
}
